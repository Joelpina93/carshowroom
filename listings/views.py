...
from .choices import price_choices, door_choices, state_choices
...
def search(request):
    ...
    if 'city' in request.GET:
        ...
        if keywords:
            queryset_list = queryset_list.filter(
                city__icontains=keywords)
    if 'state' in request.GET:
        keywords = request.GET['state']
        if keywords:
            queryset_list = queryset_list.filter(
                state__icontains=keywords)
    if 'doors' in request.GET:
        keywords = request.GET['doors']
        if keywords:
            queryset_list = queryset_list.filter(
                doors__iexact=keywords)
